...
appearance of preset styles such as "highlight", "fade", and "hide". Additionally, the `path` and root `svg` elements,
...
All CSS generated by MemoryViz, including custom themes, is scoped under the `memory-viz-diagram`
class, which is applied to the diagram's root `<svg>` element. This ensures a diagram's styles only
ever affect elements nested within that diagram, and won't leak onto unrelated content when the SVG
is embedded directly into a host page. When writing custom CSS via `--global-style` or a custom theme,
scope your selectors under `.memory-viz-diagram` (or the `svg` element itself, as shown above) rather
than targeting elements like `path` or `svg` globally.
