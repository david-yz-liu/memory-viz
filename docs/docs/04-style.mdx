...
The text in the id box can be styled with `text_id`, and the colour of the box can be styled with `box_id`.
...
Similarly, the text in the type box can be styled with `text_type` and the colour of the box can be styled with `box_type`.
...
Finally, you can style the colour of the entire box using `box_container`.
...
The user is also able to pass in a preset to style an object, as the Style data type could also be a String.
